...
Dico* tri_mot_suivant(const char *filename, const char *mot, int *taille_resultat) {
    FILE *fichier;
    Dico *resultat = NULL;
    int capacite = 10;
    ...
    *taille_resultat = 0;
    ...
    while (lire_mot(fichier, courant, sizeof(courant))) {
        if (strcmp(precedent, mot_recherche) == 0) {
            resultat = ajouter_mot(resultat, taille_resultat, &capacite, courant);
            if (resultat == NULL) {
                fclose(fichier);
                return NULL;
            }
